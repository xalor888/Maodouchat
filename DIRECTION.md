...
| instrumented 测试（androidTest） | **23 个文件** | `find app/src/androidTest -name '*.kt'`
| 自审清单体量 | 1,170,828 字节 | `wc -c docs/full-project-refactor-checklist.md`
...
| M6 | 客户端热点收敛 | **已完成（G295c）**。判据一「门禁覆盖」：`ClientArchitectureTest` **21 例 0 失败**（当轮 `--rerun-tasks` 实测，逐条列名核对），`ChatDetailRoute.kt` 3433 / `ChatDetailViewModel.kt` 3102 / `GroupPlayPolicy.kt` **1945** 与 cap **逐值相等、零余量**（两处名单同步）；判据二「旧路径真正删除」：本轮删掉 4 个确证死成员（`flipCoin`/`rollNumberGuess`/`formatAnonBox`/`formatTruthPrompt`，连带其唯一调用方 `randomTruthPrompt`），`GroupPlayPolicy.kt` 1963→1945 行，`GroupPlayPolicyTest > unreferenced members only shrink` 实测 **14→10**（断言信息 `expected:<14> but was:<10>`）。删前先跑 app 全量 **2116 例 0 失败**、server 全量 **564 例 0 失败**、新鲜度门禁 **11 例 0 失败**。 |
...
2. **剥注释本身要过负控制**：把目标符号注释掉，门禁必须变红。G157b 就是这么发现
   ...
   *（G182d/G182e 已固化成常驻测试：两侧各有一例 `stripComments itself is under test`，
   钉住「行注释被剥 / 块注释被剥 / 字符串与字符字面量里的内容必须原样保留」。）*
3. KDoc 里**不要贴** `/*`、`*/`、`//` 这些注释定界符的实例。要写就写「斜线星 / 星斜线 /
   双斜线」或转义。
   *（G182d/G182e 已固化成常驻测试：两侧各有一例
   `no test kdoc contains a literal comment delimiter`。）*

**关于第 3 条的实测修正（G182d）**：这三个符号的风险**不是并列的**。

| 符号 | 出现在 KDoc 里的后果 | 由谁强制 |
|---|---|---|
| 斜线星 | Kotlin 块注释**可嵌套**，它会**再开一层**注释 → `Unclosed comment` | **编译器** |
| 星斜线 | 提前结束注释 → `Unclosed comment` | **编译器** |
| 双斜线 | **能编译通过**，但会污染任何「按出现次数判罚」的粗粒度门禁 | **只有测试能守** |

也就是说：前两者写不进去（一写就编译失败，G182d 实测两次都是
`compileDebugUnitTestKotlin FAILED`），**测试门禁真正能守的只有双斜线**。
把三者并列会让人误以为要写三套检查——实际只有一项需要。

> 顺带一条：`//*/` 这种写法同时含斜线星与星斜线（第 2–3 字符是斜线星），
> 所以它同样会开一层嵌套注释。G182d 就是这么把一段 KDoc 写坏的。
...
#### Git pre-push 闸门（G307c / G327c）

`.githooks/pre-push` 在两道关不过时**拒绝推送**：

1. **§0 与实测不一致**——治「追加台账 / 新增文件后忘同步 §0」，
   本会话为此红过多次 CI；
2. **`:app:lintDebug` 失败**（G327c 新增）——治「lint 是我本地从不跑的那一门」：
   G325c 推送后 CI 四个作业里只有 lint 红，白白付了一整轮 CI（约 20 分钟）。
   lint 正是本地 `testDebugUnitTest`/`server test`/`connectedDebugAndroidTest`
   都不覆盖的那一门。
...
被拒时：§0 用 `--write-fast` 修（不是 `--write`：上次若是过滤跑，`--write` 会被
用例数下限守卫整体拒绝，一行都写不了）；lint 看 `/tmp/pre-push-lint.log`。
**lint 这一步冷缓存约 3 分钟、热缓存约 1 秒**——有意的代价交换。
确需跳过：`git push --no-verify`（代价自负）。详见 `scripts/README.md` §5。

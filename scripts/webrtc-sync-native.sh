...
# classpath 资源中（server/build/generated/resources/webrtc/...），首次请求时惰性解压到
# STORAGE_DIR/webrtc/ 后走 Ktor respondFile 提供 Range/断点续传。
#
# 用法：
#   1) Gradle build 已自动解压并注册资源（无需手动执行）；
#   2) 若需手动指定 .so 路径：
#        bash scripts/webrtc-sync-native.sh /path/to/libjingle_peerconnection_so.so
#
# 注意：该二进制不进版本库（源码树零二进制残留）。

set -euo pipefail

ABI="arm64-v8a"
DEST="server/build/generated/resources/webrtc/${ABI}/libjingle_peerconnection_so.so"
SRC="${1:-}"

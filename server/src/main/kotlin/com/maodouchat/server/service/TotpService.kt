...
object TotpService {
    ...
    private const val REPLAY_COUNTER_RETENTION_MS = 7L * 24L * 60L * 60L * 1_000L
    private const val REPLAY_COUNTER_SWEEP_INTERVAL_MS = 60_000L
    ...
    //（重启/多实例后不可重放）。
    private data class ReplayRecord(val counter: Long, val acceptedAtMs: Long)
    private val lastVerifiedCounter = java.util.concurrent.ConcurrentHashMap<String, ReplayRecord>()
    private var lastReplayCounterSweepAt = 0L
    ...
    /**
     * 校验 TOTP code。
     * @param trackReplay 是否写入进程内重放守卫；关闭 TOTP 等仅做时限校验的流程传 false。
    ...
    fun verify(
        ...
        maybeSweepReplayCounters(nowMs)
        ...
        for (w in -window..window) {
            ...
            if (constantTimeEquals(expected, normalized)) {
                // 先持久化（DB 原子 CAS），拒绝则整体拒绝；再内存 compute 兜底（进程内快速拒绝）
                if (!onAcceptedCounter(counter + w)) return false
                if (!trackReplay) return true
                val guardKey = secretBase32.trim()
                val newCounter = counter + w
                var updated = false
                lastVerifiedCounter.compute(guardKey) { _, old ->
                    if (old == null || newCounter > old.counter) {
                        updated = true
                        ReplayRecord(newCounter, nowMs)
                    } else {
                        old
                    }
                }
    ...
    @Synchronized
    private fun maybeSweepReplayCounters(nowMs: Long) {
        if (lastVerifiedCounter.size < MAX_REPLAY_COUNTER_ENTRIES) return
        if (nowMs - lastReplayCounterSweepAt < REPLAY_COUNTER_SWEEP_INTERVAL_MS) return
        lastReplayCounterSweepAt = nowMs
        val cutoff = nowMs - REPLAY_COUNTER_RETENTION_MS
        lastVerifiedCounter.entries.removeIf { it.value.acceptedAtMs < cutoff }
    }

...
## 待办队列（按性价比）
1. ChatDetailRoute 第十二批状态收口：延续第十批的做法，继续把会话级流程开关 / 草稿选择集收进持有类（当前 2721 行）。**注意：本仓库 app/ 在云端无法编译验证，只能静态分析 + 出建议性 PR。** 另：第十一批已于 2026-09-26 合并（#54，squash；分支已删），下一轮可启动第十二批。
...
- 2026-09-26 | 第一轮（自主循环启动）：更新 qca-progress.md 过时内容（PR #54 已合并备注），通过 PR #53（分支 `qca/maodouchat-autonomy`）把该文件落户到 main；顺带合入两份历史本地跑绿的 server 测试（CallSignalingValidatorsTest 13 例、CallSignalingGroupMetadataDbTest 4 例） | PR 分支 CI `completed/success`（server 全量 compileKotlin+test、PG 并发集成、app 编译）；自审：diff 仅 3 个新增文件，均与进度文件/已验证测试相关，无无关文件；mergeable_state=clean 后 squash 合并 | 未重跑本地测试（两份测试以前轮日志的 XML 核对结果为准，CI 全量绿即等价复检）

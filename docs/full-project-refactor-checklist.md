...
当前状态：`[~]`。`NavGraph.kt` 1050→807 行（路由定义迁入 `NavRoutes.kt`；设置 14 目的地 + 动态联系人 7 目的地迁入 `SettingsDestinations`/`ExploreDestinations` feature 簇，逐字搬运），Activity 承担较多全局状态。纯决策层已先行冻结：`AppLinkDestination` typed 目标 + `AppLinkRouter` 纯解析/清洗器（13 单测，含遍历/非法字符/危险 scheme/斜杠注入拒绝），零行为变更，待调用方渐进接入。

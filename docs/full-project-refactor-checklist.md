...
- [ ] 协议模型有向前/向后兼容与 fuzz 测试。（2026-09-26 进展：第一块——`MessagingV2Routing` 的 `messagingV2Json` 从 `ignoreUnknownKeys = false` 收敛为 `true`（其余路由早已是 `true`，此前新版客户端给 V2 发未知字段会 400），`internal` 可见性供测试直接引用同一份配置；新增 `MessagingV2ForwardCompatTest` 5 例：未知字段容忍×2、缺失可选字段回默认值、往返稳定、未知包装字段不污染已知字段。fuzz 测试仍缺，本项保持 `[ ]`。）（2026-09-27 进展：第二块——新增 `MessagingV2FuzzTest` 7 例：`SendMessageV2Request`/`AcknowledgeEnvelopesV2Request`/`ServiceMessagingV2Content` 三个线圈请求模型的种子化变异 fuzz（400+300+300 次：截断/字符翻转/词法注入/删段/跨度复制/未知嵌套，seed 固定、CI 可复跑），契约与 G20 畸形输入矩阵同款——解码器对任意输入要么成功、要么只抛 `SerializationException`，`Error`/其它异常逃逸即红；另有全量截断前缀矩阵、极端数值（超 Long 范围/`1e999`/负小数/字符串冒充数字/对象冒充数组）、64 层未知嵌套，以及"成功解码子集往返稳定"用例。V2 线圈至此向前/向后兼容 + fuzz 齐备；其余协议面的同款覆盖仍缺，本项保持 `[ ]`。）
